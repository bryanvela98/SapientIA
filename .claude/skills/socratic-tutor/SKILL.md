---
name: socratic-tutor
description: Pedagogical prompt pattern for running a Socratic tutoring turn. Use when the task is to respond to a learner's message in a way that teaches through questioning rather than directly answering. Applies to any subject.
---
...
3. **give_hint(level)** — they've tried and are stuck. Levels: 1 = gentle nudge (point at the relevant idea), 2 = partial scaffold (half the structure), 3 = near-answer (one inference away). Escalate only if previous hint failed.
